...
import numpy as np
import cadctap
from cadcutils.net import Subject, DEFAULT_REGISTRY
from cadcutils.util.utils import normalize_host
...
def _load_cert_dn(cert_path):
    with open(cert_path, 'rb') as cert_file:
        cert = crypto.load_certificate(crypto.FILETYPE_PEM, cert_file.read())
    subject = cert.get_subject()
    dn = ','.join(
        '{}={}'.format(name.decode('ascii'), value.decode('ascii'))
        for name, value in subject.get_components())
    cn = subject.CN
    if isinstance(cn, bytes):
        cn = cn.decode('ascii')
    # TAP permissions owner is the CADC username (CN without random suffix)
    owner = cn.rsplit('_', 1)[0]
    return dn, owner
...
def _test_step(name):
    ...
    logger.info('>>> STEP {:03d} START: {}'.format(step_id, name))
    sys.stdout.flush()
    sys.stderr.flush()
    try:
        yield
        ...
        logger.info('<<< STEP {:03d} DONE:  {} ({:.2f}s)'.format(
            step_id, name, elapsed))
        sys.stdout.flush()
        sys.stderr.flush()
...
def _main_app(step_name=None):
    label = step_name or _cli_label()
    with _test_step(label):
        main_app()
...
try:
    ...
    sys.exit(-1)
CERT1 = os.path.join(CERT_DIR, 'cadc-tap-test-auth1.pem')
assert os.path.exists(CERT1), 'Cannot find {}'.format(CERT1)
DN1, OWNER1 = _load_cert_dn(CERT1)
CERT2 = os.path.join(CERT_DIR, 'cadc-tap-test-auth2.pem')
assert os.path.exists(CERT2), 'Cannot find {}'.format(CERT2)
DN2, OWNER2 = _load_cert_dn(CERT2)
# not member of any group
CERTREG = os.path.join(CERT_DIR, 'cadc-tap-test-reg.pem')

# group CADC_TEST2-Staff has members: DN1 and DN2
GROUP = "CADC_TEST2-Staff"

try:
    TAP_HOST = normalize_host(os.environ['TAP_HOST'])
    logger.info('Set TAP_HOST to {}'.format(TAP_HOST))
except KeyError:
    TAP_HOST = normalize_host(DEFAULT_REGISTRY)
    logger.info('Using the default TAP_HOST {}'.format(TAP_HOST))
...
def test_astropytable():
    with _test_step('test_astropytable'):
        ...
        buffer = BytesIO()
        with _test_step('query top 1000 caom2.Observation'):
...
def test_commands(monkeypatch):
    with _test_step('test_commands'):
        ...
        with patch('sys.stdout', new_callable=BytesIO) as stdout_mock:
            _main_app('anonymous argus query')
        ...
        try:
            ...
            with patch('cadctap.core.sys.exit'):
                _main_app('delete table (cleanup)')
        ...
        try:
            _main_app('create index on article')
        ...
        sys.argv.append('select item_count, article from {}'.format(TABLE))
        with patch('sys.stdout', new_callable=StringIO) as stdout_mock:
            _main_app('query after csv load')
        ...
        sys.argv.append('select * from {}'.format(TABLE))
        with patch('sys.stdout', new_callable=BytesIO) as stdout_mock:
            _main_app('query after tsv load')
        result = stdout_mock.getvalue()
        ...
        c1 = fits.Column(name='article', array=np.array(['art6', 'art7', 'art8']),
                         format='5A')
        c2 = fits.Column(name='item_count', array=np.array([6, 7, 8]), format='K')
        ...
        sys.argv.append('select * from {}'.format(TABLE))
        with patch('sys.stdout', new_callable=BytesIO) as stdout_mock:
            _main_app('query after FITSTable load')
        result = stdout_mock.getvalue()
        assert b'<INFO name="QUERY_STATUS" value="OK" />' in result
        # 9 rows
        ...
        try:
            monkeypatch.setattr('builtins.input', lambda x: "yes")
            _main_app('delete table (final cleanup)')
            logger.debug('Deleted table {}'.format(TABLE))
        except Exception as e:
            logger.debug(
                'Cannot delete table {}. Reason: {}'.format(TABLE, str(e)))
...
def _get_permissions(cert, resource):
    ...
    sys.argv = 'cadc-tap schema --cert {} {} {}'.format(cert, HOST,
                                                        resource).split()
    with patch('sys.stdout', new_callable=StringIO) as stdout_mock:
        _main_app('schema permissions for {}'.format(resource))
    return stdout_mock.getvalue().split('\n')[-4].split()

def _create_table(monkeypatch, owner_cert):
    with _test_step('_create_table owner={}'.format(os.path.basename(owner_cert))):
        sys.argv = 'cadc-tap delete --cert {} {} {}'.format(CERT1, HOST,
                                                            TABLE).split()
        try:
            monkeypatch.setattr('builtins.input', lambda x: "yes")
            with patch('cadctap.core.sys.exit'):
                _main_app('delete table before create')
            logger.debug('Deleted table {}'.format(TABLE))
        except Exception as e:
            logger.debug(
                'Cannot delete table {}. Reason: {}'.format(TABLE, str(e)))
...
def test_permission_settings(monkeypatch):
    with _test_step('test_permission_settings'):
        with _test_step('set schema og-rw'):
        ...
        with _test_step('set schema o+r'):
            sys.argv = 'cadc-tap permission --cert {} {} o+r {}'.format(
                CERT1, HOST, DB_SCHEMA_NAME).split()
            _main_app()
        assert [OWNER1, 'true', GROUP, GROUP] == \
        ...
        with _test_step('CERT2 set table g+r'):
            sys.argv = 'cadc-tap permission --cert {} {} g+r {} {}'.format(
                CERT2, HOST, TABLE, GROUP).split()
            _main_app()
        assert [OWNER2, 'false', GROUP, '-'] == \
               _get_permissions(CERT2, TABLE)
        _check_read_permissions(TABLE, True, True, False, False)

        with _test_step('CERT2 set table o+r'):
            sys.argv = 'cadc-tap permission --cert {} {} o+r {}'.format(
                CERT2, HOST, TABLE).split()
            _main_app()
        assert [OWNER2, 'true', GROUP, '-'] == \
               _get_permissions(CERT2, TABLE)
        _check_read_permissions(TABLE, True, True, True, True)

        with _test_step('CERT2 set table og-r'):
...
def test_write_delete_permissions(monkeypatch):
    with _test_step('test_write_delete_permissions'):
        # monkeypatch required for the "user interaction"
        ...
        with _test_step('set schema og-rw'):
            sys.argv = 'cadc-tap permission --cert {} {} og-rw {}'.format(
                CERT1, HOST, DB_SCHEMA_NAME).split()
            _main_app()
        with _test_step('set schema og+r'):
            sys.argv = 'cadc-tap permission --cert {} {} og+r {} {}'.format(
                CERT1, HOST, DB_SCHEMA_NAME, GROUP).split()
            _main_app()
        ...
        with patch('sys.stdout', new_callable=StringIO) as stdout_mock:
            _main_app('schema after permission setup')
        result = stdout_mock.getvalue()

        # create table with no credentials (Fail)
        with _test_step('create table without credentials (expect fail)'):
        ...
        for cert in [CERT2, CERTREG]:
            with _test_step('create table as {} (expect fail)'.format(
        ...
        c1 = fits.Column(name='article', array=np.array(['art1']),
                         format='5A')
        c2 = fits.Column(name='item_count', array=np.array([1]), format='K')
        hdu1 = fits.BinTableHDU.from_columns([c1, c2])
        new_hdul = fits.HDUList([hdu0, hdu1])
        ...
        with _test_step('load FITSTable as CERT2 (expect fail)'):
            sys.argv = 'cadc-tap load -f FITSTable --cert {} {} {} {}'.format(
        ...
        with _test_step('load FITSTable as CERT2 (expect success)'):
            sys.argv = 'cadc-tap load -f FITSTable --cert {} {} {} {}'.format(
                CERT2, HOST, TABLE, bintb_file).split()
            try:
                _main_app()
        ...
        sys.argv.append('select * from {}'.format(TABLE))
        with patch('sys.stdout', new_callable=BytesIO) as stdout_mock:
            _main_app('query after successful load')
        result = stdout_mock.getvalue()
        assert b'<INFO name="QUERY_STATUS" value="OK" />' in result
        # 1 row
        ...
        with _test_step('load FITSTable as CERTREG (expect fail)'):
            sys.argv = 'cadc-tap load -f FITSTable --cert {} {} {} {}'.format(
        ...
        with _test_step('delete as anonymous (expect fail)'):
        ...
        for cert in [CERT2, CERTREG]:
            with _test_step('delete as {} (expect fail)'.format(
                    os.path.basename(cert))):
        ...
        with _test_step('delete as owner CERT1 (expect success)'):
            sys.argv = 'cadc-tap delete --cert {} {} {}'.format(CERT1, HOST,
                                                                TABLE).split()
            try:
                monkeypatch.setattr('builtins.input', lambda x: "yes")
...
def _check_read_permissions(table, auth1=False, auth2=False, reg=False,
                            anon=False):
    access = 'auth1={} auth2={} reg={} anon={}'.format(
        auth1, auth2, reg, anon)
    with _test_step('_check_read_permissions {}'.format(access)):
        formats = ['VOTable', 'tsv', 'csv']
        for cert, read_access in [(CERT1, auth1), (CERT2, auth2), (CERTREG, reg),
                     (None, anon)]:
            cert_label = os.path.basename(cert) if cert else 'anonymous'
            format = formats[random.randrange(0, 3)]
            step_name = 'read check cert={} format={} expect={}'.format(
                cert_label, format, 'allow' if read_access else 'deny')
            if format == 'VOTable':
                out_format = BytesIO
            else:
                out_format = StringIO
